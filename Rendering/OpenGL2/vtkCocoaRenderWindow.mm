...
void vtkCocoaRenderWindow::SetSize(int width, int height)
{
  ...
  if ((this->GetActualSizeDirectly()[0] != width) || (this->GetActualSizeDirectly()[1] != height) || this->GetParentId())
  {
...
void vtkCocoaRenderWindow::CreateAWindow()
{
  ...
  if (!this->GetRootWindow() && !this->GetWindowId() && !this->GetParentId())
  {
    ...
    {
      this->SetSizeNoEvent(static_cast<int>(NSWidth(backingScreenRect)), static_cast<int>(NSHeight(backingScreenRect)));
  ...
  if (!this->GetWindowId())
  {
    ...
    {
      NSRect backingViewRect = NSMakeRect(0.0, 0.0, (CGFloat)this->GetActualSizeDirectly()[0], (CGFloat)this->GetActualSizeDirectly()[1]);
...
const int* vtkCocoaRenderWindow::GetSize()
{
  ...
  if (this->Mapped && !this->UseOffScreenBuffers)
  {
    ...
    // Update the ivar.
    this->SetSizeNoEvent(static_cast<int>(NSWidth(backingViewRect)), static_cast<int>(NSHeight(backingViewRect)));
  }
